 ...
 * описание сущности сущность_info (brand_info)
 * контроллер оперирует моделями и получает из моделей данные к себе
 * контроллер не знает о представлении ничего и никак не влияет на его поля
 * контроллер управляет данными представления, как своими
 *
 * в представлениях:
 * представление берет данные из контроллера через __get
 * представление без крайней необходимости не использует ссылки на модели из контроллера
 *
 */
...
//вызвали рисовалку представления
	...
	static function __autoload($class_name)
	{
		...
		if (in_array($class_name, [
			'Db', 'DbPg', 'DbPgSingleton', 'DbMy',
			'Mail',
			'Model', 'SimpleDictionaryModel', 'BasicUserModel',
			'Controller', 'View', 'ViewHelper', 'ToolBar', 'Test'
		]))
		{
