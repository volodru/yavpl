 ...
 *
 * -- Контроллер
 * В приложении создается класс-контроллер, вызывается его конструктор (а в нем и все его предки)
 * Контроллеру устанавливаются поля $this->running_method_name (текущий метод), а также модуль ($__module_name) и класс ($__class_name)
 * Вызывается метод контроллера init().
 * Вызывается запрошенный метод контроллера.
 *
 * -- Представление
 * В приложении создается представление ($application->view = new Views\XXXX())
 ...
 * view->body должен быть переопределен где-то в наследниках View (как правило в Views\View проекта) и
 ...
 *
 * Контроллер ДОЛЖЕН:
 * 1. проверить аутентификацию и авторизацию.
 * 2. проверить весь пользовательский ввод (переданные через CGI или CLI параметры).
 * 3. с помощью моделей собрать данные для представления.
 *
 * Контроллер НЕ ДОЛЖЕН
 * 1. лезть в СУБД
 * 2. выводить HTML в WebUI режиме
 *
 ...
 * Случаи должны быть редкими и обоснованными. Основанием может быть одноразовость кода, либо полное отсутствие инфы о
 ...
 *
 * Модели НЕ ДОЛЖНЫ ничего знать об авторизации конкретных действий, но может использовать аутентификацию через синглтон CurrentUser.
 *
 *----------------------------------------------------------------------
 *
 * Чтобы отрисовать отдельный элемент в AJAX или iframe в контроллере нужно установить disableRender().
 *
 * Для AJAX есть специальный метод controller->isAJAX() (либо устанавливаем атрибут метода #[ajax])
 * устанавливающий заголовок с кодировкой и отключающий рендер.
 * Для работы с JSON - вызываем controller->isJSON() (либо устанавливаем атрибут метода #[json])
 * и заполняем переменную $this->result (и всё).
 *
 * Соглашение об именах в библиотеке:
 * - классы и методы - верблюжьим горбом, с маленькой буквы. Названия из более, чем одного слова надо начинать с глагола
 * - свойства и переменные маленькими буквами через подчеркивание
 *
 * В контроллерах:
 * * Экземпляр класса - название сущности во множественном числе. (brands, articles, suppliers)
 * * Идентификатор сущности сущность_id (brand_id).
 ...
 * * Контроллер не знает о представлении ничего и никак не влияет на его поля, но представление знает обо всех полях контроллера.
 *
 * В представлениях:
...
//загрузили файл и создали представление - вызвали его конструктор
		if (APPLICATION_RUNNING_MODE == 'WebUI')
		{
			...
			{
				//Тут оказываемся, если не нашёлся класс с представлением.
	...
	public static function __autoload(string $class_name): void
	{
		...
		if (preg_match("/^YAVPL\\\\/", $class_name))
		{
			...
			if (in_array($file_name, [
				'Db', 'DbPg', 'DbPgSingleton', 'DbTable', 'PostgresQL', //СУБД
				'Mail', //Почта
				'DotEnv',
