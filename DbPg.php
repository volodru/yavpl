 ...
 */
	public function connect():void
	{
		...
		{
			print "Cannot connect to PostgresQL host={$this->host_params['host']} base=[{$this->host_params['dbname']}] port=[{$this->host_params['port']}] user=[{$this->host_params['user']}] (PGSQL_CONNECT_FORCE_NEW)";
			die();
		}
...
PARAMS: ".print_r($this->query_params, true) : '').$explain);
				...
				//!!!!!!!!!!!!!
				//EXTREMELY DANGEROUS!!!! жрет память!
 ...
 * Можно либо собрать всё в память и выгрузить одной командой pg_put_line
 * + работает если сервера разные или даже с доступом по интернету
 * - двукратный расход памяти под буфер
 * Либо сразу писать pg_put_line на каждую строку входных данных
